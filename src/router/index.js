...
const router = createRouter({
  ...
  scrollBehavior(to, from, next) {
    ...
    //console.log(to.name)
    if (
      to.name !== 'worksDetailNew' &&
      to.name !== 'worksDetailOperation' &&
      to.name !== 'worksDetailFront' &&
      to.name !== 'studyDetailView'
    ) {
      let rightWrap =
        document.querySelector('.rightWrap') ||
        document.querySelector('.rightWrap2');
      return (rightWrap.scrollTop = 0);
    }
